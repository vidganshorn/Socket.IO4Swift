...
io.on('connection', function(clientSocket) {
  ...
  clientSocket.on("startType", function(deviceID) {
                  
    console.log("User " + clientSocket.id + " is writing a message...");
    
    // typingUsers[clientSocket.id] = 1;
    typingUsers[deviceID] = 1;
   
    io.emit("isTyping", typingUsers);
  });


  clientSocket.on("stopType", function(deviceID) {
    
    console.log("User " + clientSocket.id + " has stopped writing a message...");
    
    // delete typingUsers[clientSocket.id];
    delete typingUsers[deviceID];
